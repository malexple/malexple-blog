...
| :----------------------- | :----------------------------------------------------------- | :---------------- | :----------------------------------------------------------- |
| Простой ключ             | `jsonField("user.role")`                                     | `"admin"`         | Доступ к вложенному полю объекта                             |
| Индекс массива           | `jsonField("items[0].id")`                                   | `42`              | Один конкретный элемент массива по позиции                   |
| Вайлдкард (все элементы) | `jsonField("items[*].price")`                                | `[100, 200, 300]` | Собрать значения одного поля со всех элементов массива, например перед `sum()` или `avg()` |
| Фильтр по условию        | `jsonField("items[?(@.status=='active')].id")`               | `[1, 3]`          | Выбрать только те элементы массива, что удовлетворяют условию |
| Фильтр без совпадений    | `jsonField("items[?(@.status=='active')].id")` на пустом результате | `[]`              | Функция не бросает исключение — возвращает пустой список, а не null |
| Несуществующий путь      | `jsonField("user.address.city")` при отсутствии поля         | `null`            | Скрипт не падает, если промежуточное поле отсутствует        |
| Невалидное тело запроса  | `jsonField("anything")` при теле `"not a json at all"`       | `null`            | Защита от падения на запросах без JSON-тела                  |
...
| :-------------------------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| Максимальная длина скрипта              | 2000 символов                                                | `ScriptGuard.maxScriptLength` (system property `wiremockjs.max.script.length`) | При такой длине физически нельзя объявить больше ~220 переменных, поэтому отдельный MAX_VARIABLES не нужен  |
| Максимальная глубина вложенности блоков | 5 уровней                                                    | `ScriptGuard.MAX_NESTING_DEPTH`                              | Защита от искусственно раздутых деревьев разбора без лишней сложности парсера  |
| Таймаут выполнения скрипта              | 100 мс                                                       | `ScriptTransformer.EXECUTION_TIMEOUT_MS`                     | Достаточно даже с холодным стартом Faker после прогрева, но жёстко режет зависшие скрипты  |
| Циклы с произвольным условием           | Отсутствуют (нет while, for с условием)                      | Грамматика (только `forEachStatement`)                       | forEach ограничен размером готового массива — бесконечный цикл архитектурно исключён, а не просто запрещён лимитом  |
| Счётчик итераций цикла                  | Не нужен (в отличие от MAX_LOOP_ITERATIONS в Salesforce Apex) | —                                                            | Число итераций forEach равно `itemsList.size()`, которое не может превышать размер тела HTTP-запроса  |
| Пользовательские функции / eval         | Отсутствуют                                                  | Whitelist в `callWhitelistedFunction`                        | Только фиксированный набор встроенных функций — никакого произвольного кода на элемент массива  |
| Регулярные выражения                    | Только RE2J, без backreferences и части lookahead/lookbehind | `matches()` через `com.google.re2j.Pattern`                  | Гарантированное линейное время O(n), исключает ReDoS через catastrophic backtracking  |
| Переименование ключей                   | Только по статическому словарю (`mapKeys`), без вычисляемых выражений на элемент | `mapKeys(array, mapping)`                                    | Альтернатива map(item => ...) означала бы добавление eval-семантики в язык  |
| Доступ к запросу                        | Только через `RequestFacade`, без прямого объекта Request    | `visitFieldAccessExpr`                                       | request.method и подобные обращения жёстко бросают исключение |
...
https://github.com/malexple/wiremock-js-extension
